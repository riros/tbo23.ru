...
(function ($) {
    $.fn.visualLightbox = function (options) {
        var activeImage = null, badObjects = ["select", "object", "embed"], groupName = null, imageArray = [],
            slideShowTimer = null, startImage = null, descriptionHeight = 50, imgPreloader, showTimer;
        ...
        var start = this.start = function (imageLink) {
            ...
            if (options.startZoom) {
                $$("imageContainer").css({width: imageLink.width() + "px", height: imageLink.height() + "px"});
                if (!document.all) {
        ...
        function resizeImageContainer(imgTop, imgWidth, imgHeight, resizeFactor, noImage) {
            ...
            if (options.startZoom && !noImage) {
                lightboxImage2.animate($.extend({opacity: 0}, resizeFactor < 1 ? {
                    height: "120%",
                    top: "-10%",
            ...
            imageContainer.animate({width: imgWidth + "px", height: imgHeight + "px"}, {
                queue: false,
                duration: resizeDuration * 1000,
        ...
        function moveEffect(lightbox, imgTop) {
            ...
            lightbox.animate({width: "100%", left: 0, top: imgTop}, {queue: false, duration: resizeDuration * 1000});
        }
        ...
        function showImage() {
            ...
            if (options.startZoom) {
                ...
                $$("lightboxImage").css({opacity: 0}).show().fadeTo(500, 1, function () {
                    updateDetails();
                });
            }
        ...
        function updateDetails() {
            ...
            $$("imageDataContainer").animate({height: "show", opacity: "show"}, 650, null, function () {
                updateNav();
            });
        }
        ...
        function changeImageWithData(imageNum) {
            $$("imageDataContainer").animate({height: "hide", opacity: "hide"}, 650, null, function () {
        ...
        function adjustOverlay() {
            $$("overlay").css({left: getPageScroll().x + "px", top: 0, width: "100%", height: docWH()[1] + "px"});
        }
...
var Lightbox = (new $jql14).visualLightbox($VisualLightBoxParams$);